...
void UbloxParser::compute_checksum(const std::vector<uint8_t>& buf, std::size_t start, std::size_t end, uint8_t& ck_a,
                                   uint8_t& ck_b) {
    ck_a = 0;
    ck_b = 0;
...
void UbloxParser::parse_buffer() {
    while (true) {
        ...
        if (buffer_.size() < kUbxHeaderSize) return;
        ...
        if (computed_ck_a != recv_ck_a || computed_ck_b != recv_ck_b) {
            std::cerr << "Checksum error: expected (" << static_cast<int>(computed_ck_a) << ", "
                      << static_cast<int>(computed_ck_b) << ") but got (" << static_cast<int>(recv_ck_a) << ", "
                      << static_cast<int>(recv_ck_b) << "). Discarding sync byte." << std::endl;
            buffer_.erase(buffer_.begin());
            continue;
        }
        ...
        if (payloadLength > 0) {
            payload.insert(payload.end(), buffer_.begin() + kUbxHeaderSize,
