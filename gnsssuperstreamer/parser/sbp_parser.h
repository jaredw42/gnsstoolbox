...
class SwiftBinaryMessageParser {
   ...
   private:
    void parse_buffer();
    uint16_t compute_crc(const std::vector<uint8_t>& buffer, std::size_t start_index, std::size_t length,
                         uint16_t initial_crc);
    std::vector<uint8_t> buffer_;  // Changed from std::vector<char> to std::vector<uint8_t>
