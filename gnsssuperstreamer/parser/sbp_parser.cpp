...
void SwiftBinaryMessageParser::parse_buffer() {
    while (true) {
        ...
        if (buffer_.size() < kSbpHeaderSize)
            return;
        ...
        // std::cout<< "bufLen: "<< buffer_.size()<< " msg_type: " << msgType << " sender: "<< sender << " payLen: " << static_cast<int>(payloadLength) << std::endl;
